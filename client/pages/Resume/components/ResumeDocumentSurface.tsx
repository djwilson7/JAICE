...
export const ResumeDocumentSurface: React.FC<ResumeDocumentSurfaceProps> = ({
    ...
    const renderTokens = buildResumeRenderTokens(formatting);
    const { paperMetrics: paper, pageWidth, pageHeight, fieldPadding, titleStyle, headingStyle, bodyTextStyle, contactTextStyle, metaTextStyle, sectionStyle, documentCssVariables } = renderTokens;
    const innerSectionGap = "var(--resume-inner-section-gap)";
    ...
    return (
        ...
        >
            <section style={sectionStyle}>
                <h1 className="resume-document__title" style={titleStyle}>
            ...
            {resumeData.summary && (
                <section style={sectionStyle}>
                    <h2 className="resume-document__section-title" style={headingStyle}>{getSectionTitle(resumeData, "summary")}</h2>
                    <p className="resume-document__body" style={{ ...bodyTextStyle, margin: 0, padding: fieldPadding }}>{resumeData.summary}</p>
                </section>
            )}
            ...
            {(resumeData.experience || []).some((exp) =>
                ...
                <section style={sectionStyle}>
                    <h2 className="resume-document__section-title" style={headingStyle}>{getSectionTitle(resumeData, "experience")}</h2>
                    <div style={{ display: "flex", flexDirection: "column", gap: innerSectionGap }}>
                                            ...
                                            <div style={{ display: "flex", flexDirection: "column", gap: RESUME_DOCUMENT_TYPOGRAPHY.educationDetailGapPx, margin: `0 0 0 ${RESUME_DOCUMENT_TYPOGRAPHY.bulletIndentPx}px` }}>
                                                {(exp.bullets || []).filter((bullet) => hasText(bullet.text)).map((bullet) => (
                                                    <div key={bullet.id} className="resume-diagnostic-bullet-row" data-resume-diagnostic="bullet-row" style={bulletRowStyle}>
                                                        ...
                                                        <div className="resume-document__body" style={bulletTextStyle}>{bullet.text}</div>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
            ...
            {(resumeData.education || []).some((ed) =>
                ...
                <section style={sectionStyle}>
                    <h2 className="resume-document__section-title" style={headingStyle}>{getSectionTitle(resumeData, "education")}</h2>
                    <div style={{ display: "flex", flexDirection: "column", gap: innerSectionGap }}>
                                            ...
                                            <div style={{ display: "flex", flexDirection: "column", gap: RESUME_DOCUMENT_TYPOGRAPHY.educationDetailGapPx, margin: `${RESUME_DOCUMENT_TYPOGRAPHY.educationDetailGapPx}px 0 0 ${RESUME_DOCUMENT_TYPOGRAPHY.bulletIndentPx}px` }}>
                                                {details.map((detail) => (
                                                    <div key={detail.id} className="resume-diagnostic-bullet-row" data-resume-diagnostic="bullet-row" style={bulletRowStyle}>
                                                        <span style={bulletMarkerStyle}>&bull;</span>
                                                        <div className="resume-document__body" style={bulletTextStyle}>{detail.text}</div>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
            ...
            {(resumeData.skills || []).some((skill) => hasText(skill.category) || (skill.items || []).some(hasText)) && (
                <section>
                    <h2 className="resume-document__section-title" style={headingStyle}>{getSectionTitle(resumeData, "skills")}</h2>
                    <div style={{ display: "flex", flexDirection: "column", gap: innerSectionGap }}>
                        {(resumeData.skills || [])
                            ...
                            .map((skill) => (
                                <div key={skill.id} style={{ ...bodyTextStyle, display: "flex", alignItems: "flex-start", justifyContent: "flex-start", gap: RESUME_DOCUMENT_TYPOGRAPHY.metaGroupGapPx }}>
                                    {hasText(skill.category) && <strong className="resume-document__meta" style={{ flexShrink: 0, padding: fieldPadding, color: "#0f172a", fontSize: "var(--resume-subheader-font-size)", fontFamily: RESUME_DOCUMENT_TYPOGRAPHY.contactFamily, fontWeight: 700, lineHeight: "var(--resume-subheader-line-height)", textAlign: "left" }}>{skill.category}</strong>}
