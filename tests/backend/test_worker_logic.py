...
def test_run_classification_model_uses_rules_before_deberta(monkeypatch):
    def fail_if_called(_text):
        raise AssertionError("DeBERTa fallback should not run for clear rule matches")

    monkeypatch.setattr(class_tasks, "classify_email_stage", fail_if_called)
...
def test_run_classification_model_falls_back_to_deberta_for_ambiguous(monkeypatch):
    calls = []
    ...
    def fake_classify(text):
        ...
        return {
            ...
            "second_score": 0.1,
            "stage_scores": {"applied": 0.82, "interview": 0.1},
        }
    ...
    assert calls
    assert [item["email_id"] for item in result.applied] == ["fallback-1"]
